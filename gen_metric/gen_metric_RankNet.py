   ...
   # def accuracy(self,pred):
    ...
    def roc(self,pred,pred_o):
        ...
        for key in pred.keys():
            ...
            y_test=[int(i in list(self.combdic[key][:30])) for i in pred[str(float(int(key)+5181))]]
            y_probs+=y_prob
            y_tests+=y_test
            break
    ...
    def pred_path(self, path):#这个函数和下面那个函数有什么区别
        ...
        res = {}
        for fn in fnames:
            if '.json' in fn and 'predictions_v1_BERT' in fn and 'event' not in fn:
                C=float(fn.split("_")[3])
                test_file=float(fn.split('_')[4].replace('.json',''))
                print(fn)
            else:
                continue
...
if __name__ == "__main__":
    ...
    #met.pred_single_path('/Users/yuqi/Downloads/RankSVM_for_SLR/svm_rank/prediction_bert_ranksvm_0.01_.json')
    met.pred_single_path('/Users/yuqi/Downloads/RankSVM_for_SLR/Bert-test-1_epoch-4.json')
