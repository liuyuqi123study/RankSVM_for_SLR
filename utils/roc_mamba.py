...
with open ('RankMamba/features/prediction_0_1.dat') as f1:
    for line in f1:
...
score_mamba_train = pd.read_csv('RankMamba/Performances/train_score_mamba0.csv')
...
with open('RankMamba/features/train_feature_modified0.dat') as f:
    for line in f:
                    score_svm_train.loc[j,'cid']=int(line.split('#')[1])
...
with open ('RankMamba/features/prediction_0_1.dat') as f2:
    for line in f2:
        S_color_2_test.append(float(line.strip()))
#对这种方法下的得分进行标识
#fifth step: get the result of the mamba on the test dataset

score_mamba_test = pd.read_csv('RankMamba/features/test_score_mamba0.csv')
...
with open('RankMamba/features/test_feature_modified0.dat') as f:
    for line in f:
                    score_svm_test.loc[j,'cid']=int(line.split('#')[1])
...
plt.figure(figsize=(8, 6))
plt.plot(fpr_test_mamba, tpr_test_mamba, color='blue', lw=2, label=f'ROC Curve for Lawformer(AUC = {roc_auc_test_mamba:.2f})')
