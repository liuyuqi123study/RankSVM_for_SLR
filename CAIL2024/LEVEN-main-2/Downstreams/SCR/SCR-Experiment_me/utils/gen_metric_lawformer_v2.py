...
class Metric:
    ...
    def pred_path(self, path):
        ...
        res = {'MAP':0,'Best':-1,'recall':0}
        for fn in fnames:
            if 'v2_lawformer' not in fn or '.json' not in fn:
                continue
            fsp = fn.split("_")
            C = fsp[-1].replace('.json','')
            metric = {}
...
if __name__ == "__main__":
    ...
    # vanilla BERT
    print('vanilla BERT')
    met.pred_path('/Users/yuqi/Downloads/svm_rank/LeCaRDv2')   # path to the predicted files
