...
class JsonFromFilesDataset(Dataset):
    def __init__(self, config, mode, encoding="utf8", *args, **params):
        ...
        self.mode = mode
        self.file_list = []
        self.data_path = config.get("data", "%s_data_path" % mode)#在这里获取对应文件的路径
        self.encoding = encoding

        filename_list = config.get("data", "%s_file_list" % mode).replace(" ", "").split(",")#在这里要找到参数mode_file_list
        ...
        self.file_list.sort()#进行深度搜索得到所有的list列表
