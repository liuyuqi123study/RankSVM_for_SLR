...
class PairwisePLM(nn.Module):#config为读取的配置文件
    def __init__(self, config, gpu_list, *args, **params):
        ...
        if config.getboolean('model', 'use_event'):
            ...
            self.encoder = EventBertModel.from_pretrained(plm_path)#进行模型的初始化
        else:
            print('\nusing original BERT')
            self.encoder = BertModel.from_pretrained(plm_path)#使用原始的bert模型
        self.plm_config = AutoConfig.from_pretrained(plm_path)#这个使用的模型就是bert-base-Chinese
