...
def init_formatter(config, task_list, *args, **params):#对formatter进行初始化
...
def init_one_dataset(config, mode, *args, **params):
    ...
    if mode != "train":
        try:
            ...
            temp_mode = "train"
    which = config.get("data", "%s_dataset_type" % temp_mode)#获得data_section,temp_mode这个选项的参数
    ...
    if which in dataset_list:
        ...
        dataloader = DataLoader(dataset=dataset,
                                ...
                                drop_last=drop_last)
                                #sampler=sampler)#关键是这里的数据加载设置出了问题
...
def init_dataset(config, *args, **params):
    ...
    valid_dataset = init_one_dataset(config, "valid", *args, **params)#对这两个数据集的初始化是通过config实现的
