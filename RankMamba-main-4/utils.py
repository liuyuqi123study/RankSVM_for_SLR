...
def flatten_concatenation_batch(batch):
    flat_list=[]
    for row in batch:
        flat_list+=row['inputs']
    return flat_list
...
def read_queries(fname, q_prefix="[query]"):
    ...
    with open(fname, "r") as fin:
        ...
        for que in q:
            qid=str(que['ridx'])
            query = que['q']
        test_queries[qid] = q_prefix+query
    return test_queries

def configure_eval_dataset(name, is_autoregressive=False):
    if name == "dev":
        input_dir = "/content/drive/MyDrive/LEVEN-main/Downstreams/SCR/SCR-Experiment/input_data/query/query_5fold"
        test_queries = os.path.join(input_dir, "query_0.json")
        #bm25_ranklist = os.path.join(input_dir, "bm25-ranklist.dev.run")
        ranklist = 'dataset.csv'
