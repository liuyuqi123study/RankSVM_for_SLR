...
#然后我们进行绘制
fpr_train_BERT,tpr_train_BERT,thresholds=roc_curve(score_BERT_train['label'].tolist(),score_BERT_train['score'].tolist())
...
plt.plot(fpr_train_BERT, tpr_train_BERT, color='blue', lw=2, label=f'ROC Curve for BERT(AUC = {roc_auc_train_BERT:.2f})')
...
#然后我们进行绘制
fpr_test_BERT,tpr_test_BERT,thresholds=roc_curve(score_BERT_test['label'].tolist(),score_BERT_test['score'].tolist())
...
plt.figure(figsize=(8, 6))
plt.plot(fpr_test_BERT, tpr_test_BERT, color='blue', lw=2, label=f'ROC Curve for BERT(AUC = {roc_auc_test_BERT:.2f})')
