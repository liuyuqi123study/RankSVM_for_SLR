...
class Metric:
    ...
    def NDCG(self, pred, K):
        ...
        for key in pred.keys():
            ...
            rawranks = [self.avglist[key][str(i)] for i in pred[key] if str(i) in list(self.avglist[key].keys())]
    ...
    def MAP(self, pred):
        smap = 0.0
        for key in pred.keys():
            ranks = [i for i in pred[key] if str(i) in list(self.avglist[key].keys())]
    ...
    def pred_path(self, path):#这个函数和下面那个函数有什么区别
        ...
        res ={"MAP": -1, "best": -1,'recall':0}
        for fn in fnames:
            if '.json' in fn:
                Epoch=fn.split('_')[1].split('-')[1].replace('.json','')
            else:
                continue
...
if __name__ == "__main__":
    ...
    met.pred_path('/Users/yuqi/Library/CloudStorage/GoogleDrive-yuqi5341@gmail.com/我的云端硬盘/CAIL2024/LEVEN-main-2/Downstreams/SCR/SCR-Experiment_me/result/BERT/LEVEN')
