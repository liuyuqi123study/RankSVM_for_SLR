...
class BertCrfTagging(object):
    def __init__(self,
                 data_path='/content/driver/My\ Drive/cail2024/input_data/query/train_query_fact.json',
                 save_path='/content/driver/My\ Drive/cail2024/output_data-supervised/query/test_query_fact.json',
...
if __name__ == "__main__":
    ...
    os.makedirs('/content/output_data-supervised/candidates', exist_ok=True)

    # process query data
    model = BertCrfTagging(data_path='input_data/query/train_query_fact.json',
                           save_path='./output_data-supervised/query/train_query.json',
                           max_length=100,
                           batch_size=50,
                           mode='query',
    ...
    model = BertCrfTagging(data_path='/content/candidate/candidate_fact_0608',
                           save_path='/content/output_data-supervised/candidates',
                           max_length=409,
                           batch_size=50,
                           mode='candidate',
                           device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    #process test data
    model = BertCrfTagging(data_path='input_data/query/test_query_fact.json',
                           save_path='./output_data-supervised/query/test_query.json',
                           max_length=100,
                           batch_size=50,
                           mode='query',
                           device=torch.device("cuda" if torch.cuda.is_available() else 'cpu'))
